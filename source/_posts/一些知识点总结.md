...
* 如果目前全部是活跃的链接（如何看是不是活跃？ 看realconnection里的list<weakreference<streamallocation>> 删除为null的引用，看list里数量是不是为0 不是的话表示当前连接还在被使用。）
* 如果一个活跃的链接都没有 返回-1 跳出死循环、
...
### okhttp 三个重要的类： 
[参考该链接](https://cloud.tencent.com/developer/article/1199053)
