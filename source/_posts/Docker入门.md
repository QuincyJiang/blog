title: Docker入门
date: 2018/05/30 15:56:50
categories: 容器技术
comments: true
tags: [docker,容器]
---
Docker是目前最火的Linux容器解决方案，真正做到一劳永逸的解决环境配置和依赖问题，而且可以方便的修改、分享，版本管理，本篇文章摘选自 **阮一峰的博客**：[Docker 入门教程
...
上面代码中，docker image pull是抓取 image 文件的命令。`library/hello-world`是 image 文件在仓库里面的位置，其中library是 image 文件所在的组，`hello-world`是 image 文件的名字。
...
### 10.2 创建 image 文件
有了 Dockerfile 文件以后，就可以使用docker `image build`命令创建 image 文件了。
...
注意，指定了CMD命令以后，`docker container run`命令就不能附加命令了（比如前面的/bin/bash），否则它会覆盖CMD命令。现在，启动容器可以使用下面的命令。
...
首先，去 [cloud.docker.com](hub.docker.com) 或 [cloud.docker.com]() 注册一个账户。然后，用下面的命令登录。
